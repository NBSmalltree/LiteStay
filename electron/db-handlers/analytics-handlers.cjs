...
function registerHandlers(ipcMain, getDb, getMainWindow) {
  ...
  ipcMain.handle('db:getDailyOccupancy', (_event, dateFrom, dateTo) => {
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      const occupied = new Set(orders.filter(o => o.check_in_date <= dateStr && o.check_out_date > dateStr).map(o => o.room_id)).size;
      result.push({ date: `${d.getMonth() + 1}/${d.getDate()}`, totalRooms, occupiedRooms: occupied, occupancyRate: totalRooms > 0 ? Math.round((occupied / totalRooms) * 100) : 0 });
    }
    return result;
  });
...
GROUP BY DATE(fl.created_at), r.room_type ORDER BY date, room_type`).all(dateFrom, dateTo);
  });
...
ELSE julianday(check_out_date, 'start of day') - julianday(check_in_date, 'start of day') END) as nights
FROM orders WHERE status != 'CANCELLED' AND check_in_date < ? AND check_out_date > ?`)
      .get(dateTo, dateFrom, dateTo, dateFrom, dateFrom, dateFrom, dateFrom, dateTo, dateFrom);
    const totalRoomFee = db.prepare(`SELECT SUM(amount) as total FROM financial_logs WHERE type = 'ROOM_FEE' AND DATE(created_at) BETWEEN ? AND ?`).get(dateFrom, dateTo);
    ...
    return {
      adr: nights > 0 ? round2(roomFee / nights) : 0,
      revpar: availableRoomNights > 0 ? round2(roomFee / availableRoomNights) : 0,
      total_room_fee: roomFee, sold_room_nights: Math.round(nights),
      available_room_nights: availableRoomNights,
      occupancy_rate: availableRoomNights > 0 ? round2(nights / availableRoomNights * 100) : 0,
    };
  });
  ...
  ipcMain.handle('db:getADRRevPARTrend', (_event, days) => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dateStr = d.toISOString().slice(0, 10);
      const occupied = db.prepare(`SELECT COUNT(DISTINCT room_id) as c FROM orders WHERE status = 'IN_HOUSE' AND check_in_date <= ? AND check_out_date > ?`).get(dateStr, dateStr);
      const roomFee = db.prepare(`SELECT SUM(amount) as total FROM financial_logs WHERE type = 'ROOM_FEE' AND DATE(created_at) = ?`).get(dateStr);
      const occupiedCount = occupied?.c || 0;
      const fee = roomFee?.total || 0;
      const round2 = (v) => Math.round(v * 100) / 100;
      result.push({
        date: `${d.getMonth() + 1}/${d.getDate()}`,
        adr: occupiedCount > 0 ? round2(fee / occupiedCount) : 0,
        revpar: totalRooms > 0 ? round2(fee / totalRooms) : 0,
        occupancy_rate: totalRooms > 0 ? round2(occupiedCount / totalRooms * 100) : 0,
      });
    }
    return result;
  });
...
ELSE julianday(o.check_out_date, 'start of day') - julianday(o.check_in_date, 'start of day') END`;
    return getDb().prepare(`SELECT r.room_type, COUNT(DISTINCT o.order_id) as order_count, SUM(o.actual_amount) as total_revenue,
SUM(${nightCase}) as total_nights, AVG(o.actual_amount / ${nightCase}) as avg_adr
FROM orders o JOIN rooms r ON o.room_id = r.room_id
WHERE o.status != 'CANCELLED' AND o.check_in_date < ? AND o.check_out_date > ?
GROUP BY r.room_type ORDER BY avg_adr DESC`)
  ...
  // Revenue Analytics: type-summary CASE fragment
...
GROUP BY strftime('%Y-%m', fl.created_at) ORDER BY month`).all(year.toString());
  });
...
SUM(fl.amount) as total, ${typeSum('ROOM_FEE', 'room_fee')}, ${typeSum('DEPOSIT', 'deposit')}, ${typeSum('INCIDENTAL', 'incidental')}
FROM financial_logs fl WHERE strftime('%Y', fl.created_at) = ? GROUP BY quarter ORDER BY quarter`).all(year.toString());
  });
...
FROM financial_logs fl GROUP BY strftime('%Y', fl.created_at) ORDER BY year DESC LIMIT 5`).all();
  ...
  ipcMain.handle('db:getRevenueGrowth', () => {
    ...
    const getTotal = (month) => getDb().prepare(`SELECT SUM(amount) as total FROM financial_logs WHERE strftime('%Y-%m', created_at) = ?`).get(month)?.total || 0;
    ...
    return { current_month: currentTotal, last_month: lastTotal, growth_rate: Math.round(growth * 100) / 100, growth_amount: currentTotal - lastTotal };
  });
...
GROUP BY month, source ORDER BY month, source`).all(months);
  });
