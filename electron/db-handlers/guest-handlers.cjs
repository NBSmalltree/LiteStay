...
function registerHandlers(ipcMain, getDb) {
  ...
  ipcMain.handle('CH.updateGuest', (_event, guestId, updates) => {
    const db = getDb();
  ...
  ipcMain.handle('CH.deleteGuest', (_event, guestId) => {
  ...
  ipcMain.handle('CH.getGuestsWithStats', () => {
...
FROM orders o JOIN rooms r ON o.room_id = r.room_id WHERE o.guest_name = ? ORDER BY o.check_in_date DESC`).all(guestName);
  ...
  ipcMain.handle('CH.findOrCreateGuest', (_event, guestData) => {
    const db = getDb();
    if (guestData.phone) {
