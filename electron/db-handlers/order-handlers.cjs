...
const { CH, buildUpdateQuery, notifyOrdersChanged } = require('./utils.cjs');

function registerHandlers(ipcMain, getDb, getMainWindow) {

  ipcMain.handle('CH.insertOrder', (_event, order) => {
...
FROM orders o LEFT JOIN guests g ON o.guest_id = g.guest_id ORDER BY o.check_in_date DESC`).all();
  ...
  ipcMain.handle('CH.updateOrder', (_event, orderId, updates) => {
  ...
  ipcMain.handle('CH.deleteOrder', (_event, orderId) => {
