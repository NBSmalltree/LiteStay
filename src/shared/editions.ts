...
/**
 * Check if a given edition has access to a specific feature
 */
export function hasFeature(edition: Edition, feature: FeatureKey): boolean {
  return FEATURE_MAP[feature]?.includes(edition) ?? false
}
...
/**
 * Check if current edition is at least the required level
 */
